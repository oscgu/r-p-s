...
describe("Rock, Paper, Scissors", function () {
  ...
  describe("resolveWagerP2", function() {
    it("Shouldn't let p2 resolve the wager if timer is still running", async function() {
      ...
      await expect(await rps.connect(p2).resolveWagerP2(p1.address, wagerIndex)).to.revertedWith("Timer didn't run out yet");
    });
    ...
    it("Shouldn't let p2 resolve the wager if wager doesn't have a second player", async function() {
      ...
      await expect(await rps.connect(p2).resolveWagerP2(p1.address, wagerIndex)).to.revertedWith("Wager doesn't have a second player");
    });
    ...
    it("Should clean up after resolve", async function() {
      ...
      await expect(await rps.connect(p1).listWager(p1.address, wagerIndex)).to.be.revertedWith("Index out of bounds");
    });
  });
